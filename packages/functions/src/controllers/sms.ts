...
import { INotification, IOvertime, IShift, ISickRequest, ITeam, ITimeOff, ITimeOffRequest, ITradeRequest, IUser, NotificationType } from 'coverme-shared';
...
const handleSickSmsReplyApprove = async (smsInfo: string) => {
    ...
    try {
		...
		await dbHandler.addDocument<IOvertime>(
			'overtime-callouts',
			{
                shiftId: shift.id,
