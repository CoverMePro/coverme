...
const OvertimeConfirmationView: React.FC = () => {
	...
	useEffect(() => {
		...
		api.getData<IOvertime>(`overtime-callouts/${overtimeId}/${user}/info`)
			.then((overtimeInfoResult) => {
				setOvertimeCallout(overtimeInfoResult);
			})
			.catch((err) => {
	...
	const handleAcceptCallout = () => {
		if (overtimeCallout) {
			api.post(`overtime-callouts/${overtimeCallout.id}/accept`, {
	...
	const handleRejectCallout = () => {
		if (overtimeCallout) {
			api.post(`overtime-callouts/${overtimeCallout.id}/reject`, {
				email: searchParams.get('user'),
			})
				.then(() => {
					setCompleted(true);
